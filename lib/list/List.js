...
var List = /** @class */ (function() {
  ...
  List.prototype.getItemsByCaml = function(caml) {
    ...
    return this._dao
      .authorizedPost(queryUrl, postBody)
      .then(utils.validateODataV2);
  };
  ...
  List.prototype.getItemsByView = function(viewName) {
    var _this = this;
    var viewUrl =
      this.baseUrl + "/Views/getByTitle('" + viewName + "')/ViewQuery";
    // 1. Get the targeted view on the targeted list so we can pull out the ViewXml
    return this._dao
      .get(viewUrl)
      .then(utils.validateODataV2)
      .then(function(view) {
        return _this.getItemsByCaml(view.ViewQuery);
      });
  };
  ...
  List.prototype.updateItem = function(itemId, updates, digest) {
    ...
    return this._dao.auth.ensureRequestDigest(digest).then(function(digest) {
      return _this.getItemById(itemId).then(function(item) {
        ...
        var customOptions = {
          headers: utils.headers.getUpdateHeaders(
            digest,
  ...
  List.prototype.deleteItem = function(itemId, digest) {
    ...
    return this._dao.auth.ensureRequestDigest(digest).then(function(digest) {
      return _this.getItemById(itemId).then(function(item) {
        var customOptions = {
          headers: utils.headers.getDeleteHeaders(
            digest,
            item["__metadata"].etag
          )
        };
    ...
    return this._dao.auth.ensureRequestDigest(digest).then(function(digest) {
      return utils.getArrayBuffer(attach).then(convertedFile => {
        ...
        var attachmentAddUri =
          ...
          attach.name +
          "')";
        return utils.loadScript("/_layouts/15/SP.RequestExecutor.js").then(() => {
          return new Promise(function(resolve, reject) {
            var executor = new SP.RequestExecutor(url === "" ? "/" : url);
  ...
  List.prototype.addFileToLibrary = function(attach, name, digest) {
    var _this = this;
    return new Promise(function(resolve, reject) {
      _this._dao.auth.ensureRequestDigest(digest).then(function(digest) {
        utils.getArrayBuffer(attach).then(function(buffer) {
          var decoded = arrayBufferToString(buffer);
          var url = _this._dao.webUrl.replace(` `, ``);
          var attachmentAddUri =
            (url !== "" ? url + "/" : "") +
            "_api" +
          ...
          utils.loadScript("/_layouts/15/SP.RequestExecutor.js").then(() => {
            var executor = new SP.RequestExecutor(url === "" ? "/" : url);
            executor.executeAsync({
              url: attachmentAddUri,
              method: "POST",
